...
describe("ocr target queries", () => {
  it("compiles single-token queries without a near query", () => {
    const { queryExpression, ocrBindings, itemPredicates } = buildQueryPlan({
      queries: {
        target: "ocr",
        value: "CAPPAERT",
        matchMode: "includes",
    ...
    expect(queryExpression).toBe("cts:true-query()");
    expect(itemPredicates).toBe("[@uuid = $ocrUuids1]");
    expect(ocrBindings).toHaveLength(1);
    expectContainsAll(ocrBindings[0]?.expression ?? "", [
      'cts:element-query(xs:QName("ocr")',
      'cts:word-query("cappaert"',
      "return document-uri($ocrDocument)",
    ]);
    expectContainsNone(ocrBindings[0]?.expression ?? "", ["cts:near-query"]);
  });

  it("compiles exact single-token queries as whole-word value equality", () => {
    const { ocrBindings } = buildQueryPlan({
      queries: {
        target: "ocr",
        value: "laude",
        matchMode: "exact",
  ...
  // `cts:uris` resolves from indexes only and reports phrase false positives.
  it("resolves matching documents through a filtered search, not cts:uris", () => {
    const { ocrBindings } = buildQueryPlan({
      queries: {
        target: "ocr",
    ...
    expectContainsAll(ocrBindings[0]?.expression ?? "", [
      "for $ocrDocument in cts:search(doc(),",
      "return document-uri($ocrDocument)",
    ]);
    expectContainsNone(ocrBindings[0]?.expression ?? "", ["cts:uris("]);
  });

  it("compiles multi-token queries as ordered adjacency spanning all tokens", () => {
    const { ocrBindings } = buildQueryPlan({
      queries: {
        target: "ocr",
        value: "magna cum laude",
    ...
    const expression = ocrBindings[0]?.expression ?? "";

    expectContainsAll(expression, [
      'cts:word-query("magna"',
      'cts:word-query("cum"',
      'cts:word-query("laude"',
      '), 2, ("ordered"))',
    ]);
    expect(countOccurrences(expression, "cts:near-query((")).toBe(1);
  });

  it("keeps stop words so phrase adjacency stays accurate", () => {
    const { ocrBindings } = buildQueryPlan({
      queries: {
        target: "ocr",
  ...
  it("returns a false query when a value has no searchable tokens", () => {
    const { ocrBindings } = buildQueryPlan({
      queries: {
        target: "ocr",
        value: "***",
    ...
    expect(ocrBindings[0]?.expression).toContain("cts:false-query()");
  });

  it("negates through the item predicate instead of cts:not-query", () => {
    const { queryExpression, itemPredicates } = buildQueryPlan({
      queries: {
        target: "ocr",
        value: "laude",
        matchMode: "exact",
    ...
    expect(itemPredicates).toBe("[not(@uuid = $ocrUuids1)]");
    expectContainsNone(queryExpression ?? "", ["cts:not-query"]);
  });

  it("shares one binding across repeated leaves and separates distinct ones", () => {
    const { ocrBindings, itemPredicates } = buildQueryPlan({
      queries: {
        and: [
          {
            target: "ocr",
    ...
    expect(ocrBindings.map((binding) => binding.name)).toStrictEqual([
      "$ocrUuids1",
      "$ocrUuids2",
    ]);
    expect(itemPredicates).toBe("[@uuid = $ocrUuids1][@uuid = $ocrUuids2]");
  });

  it("rejects leaves nested inside a multi-branch OR group", () => {
    expect(() =>
      buildQueryPlan({
        queries: {
          or: [
    ...
    ).toThrow(OCR_DISJUNCTION_ERROR_MESSAGE);
  ...
  it("allows leaves under a single-branch OR group, which compiles to its child", () => {
    const { itemPredicates } = buildQueryPlan({
      queries: {
        or: [
          {
            target: "ocr",
...
describe("fetchSetItems query assembly", () => {
  ...
  it("binds OCR document URIs before the search and filters the inline item path", async () => {
    const postedBody = await captureSetItemsQuery({
      setScopeUuids: [SET_UUID],
      queries: {
        and: [
    ...
    expectContainsAll(postedBody, [
      "let $ocrUuids1 := for $ocrDocument in cts:search(doc(),",
      'cts:element-query(xs:QName("ocr")',
      "let $items := cts:search(doc()/ochre/set[@uuid = $setScopeUuids]/items/*[@uuid = $ocrUuids1], $query)",
    ]);
    expect(postedBody.indexOf("let $ocrUuids1 :=")).toBeLessThan(
      postedBody.indexOf("let $items :="),
    );
  });

  it("rejects OCR queries nested inside an OR group before fetching", async () => {
    let didFetch = false;
    ...
    expect(result.detailedError).toContain(OCR_DISJUNCTION_ERROR_MESSAGE);
  });

  it("rejects a language field on OCR queries", async () => {
    const result = await fetchSetItems(
      {
        setScopeUuids: [SET_UUID],
        queries: {
      ...
      undefined,
      { fetch: async () => new Response("<result><ochre/></result>") },
    );
...
describe("fetchSetPropertyValues query assembly", () => {
  ...
  it("applies OCR item filters so facet counts match the filtered items", async () => {
    const postedBody = await captureSetPropertyValuesQuery({
      setScopeUuids: [SET_UUID],
      queries: {
        target: "ocr",
      ...
      attributes: { bibliographies: false, periods: true },
    });

    expectContainsAll(postedBody, [
      "let $ocrUuids1 := for $ocrDocument in cts:search(doc(),",
      "doc()/ochre/set[@uuid = $setScopeUuids]/items/*[@uuid = $ocrUuids1]",
    ]);
  });
});
